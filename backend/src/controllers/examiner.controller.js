...
const ExaminerController = {
    ...
    getDashboardStats: async (req, res) => {
        try {
            ...
            // FIX: Examiner only counts submissions that have passed leader review (status = 'PENDING')
            // LEADER_PENDING submissions are invisible to examiners — they haven't cleared stage 1 yet
            const [pRaw] = await pool.query(
                `SELECT COUNT(DISTINCT bs.id) as total FROM badge_submissions bs ${whereClause} bs.status = 'PENDING'`,
                params
            );
            const [aRaw] = await pool.query(
                `SELECT COUNT(DISTINCT bs.id) as total FROM badge_submissions bs ${whereClause} bs.status = 'APPROVED'`,
                params
            );
            const [rRaw] = await pool.query(
                `SELECT COUNT(DISTINCT bs.id) as total FROM badge_submissions bs ${whereClause} bs.status = 'REJECTED'`,
                params
            );
            ...
            const pending = pRaw[0]?.total || 0;
            const approved = aRaw[0]?.total || 0;
            const rejected = rRaw[0]?.total || 0;
    ...
    // FIX: Get pending badges for examiner evaluation
    // Only shows submissions with status = 'PENDING' — meaning the leader has already approved them.
    // LEADER_PENDING submissions do NOT appear here.
        ...
        try {
            ...
            let query = `
                SELECT bs.*, b.badge_name as name, b.description, b.level_name as badge_level,
                        ...
                        lu.full_name as leader_approved_by
                 FROM badge_submissions bs
                 JOIN badges b ON bs.badge_id = b.id
                 JOIN scouts s ON bs.scout_id = s.id
                 JOIN users u ON s.user_id = u.id
                 LEFT JOIN users lu ON lu.id = bs.leader_approved_by_user_id
                 WHERE bs.status = 'PENDING'
                 AND bs.id IN (
                     SELECT MIN(id) FROM (
                         SELECT id, scout_id, badge_id FROM badge_submissions
                     ) as sub GROUP BY scout_id, badge_id
                 )
            `;
            let params = [];
    ...
    // Approve badge (final examiner approval)
    approveBadge: async (req, res) => {
        try {
            ...
            const [info] = await pool.query(
                `SELECT bs.scout_id, b.badge_name as badge_name, b.id as badge_id,
                        u.email as scout_email, u.full_name as scout_name
                 FROM badge_submissions bs
                 JOIN badges b ON bs.badge_id = b.id
                 JOIN scouts s ON bs.scout_id = s.id
                 JOIN users u ON s.user_id = u.id
            ...
            // Guard: examiner cannot approve something that hasn't cleared leader stage
            const [submission] = await pool.query("SELECT status FROM badge_submissions WHERE id = ?", [id]);
            if (submission[0].status === 'LEADER_PENDING') {
                return res.status(403).json({
                    message: "This submission has not yet been reviewed by the scout leader."
                });
            }
            ...
            await pool.query(
                `UPDATE badge_submissions
                 SET status = 'APPROVED', reviewed_at = NOW(),
                     reviewed_by_examiner_user_id = ?, examiner_comment = ?
                 WHERE id = ?`,
                [examiner_user_id, feedback || 'Badge approved', id]
            );
 
            // Sync badge progress to COMPLETED
            ...
            if (io) {
                ...
                // President's Award graduation check
    ...
    // Reject badge (examiner final rejection)
    rejectBadge: async (req, res) => {
        try {
            ...
            // Guard: don't allow rejecting something still at leader stage
            const [submission] = await pool.query("SELECT status, scout_id FROM badge_submissions WHERE id = ?", [id]);
            if (!submission.length) return res.status(404).json({ message: "Submission not found" });
            if (submission[0].status === 'LEADER_PENDING') {
                return res.status(403).json({
                    message: "This submission has not yet been reviewed by the scout leader."
                });
